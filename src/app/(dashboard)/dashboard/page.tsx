...
export default function DashboardPage() {
  ...
  const now = new Date();
  const monthName = now.toLocaleString(lang === "en" ? "en-US" : "tr-TR", { month: "long", year: "numeric" });
  ...
  return (
    <div>
      ...
      <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-wide mt-6 mb-3">{t.sectionInvoices}</h2>
      ...
      <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-wide mt-8 mb-3">{t.sectionVat}</h2>
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <KPICard
          ...
          icon={BarChart3}
          color={data.vat.estimatedCarryForwardVat > 0 ? "green" : "gray"}
        />
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <KPICard
          ...
          subtitle={t.rateWith(Math.round(data.provisional.taxRate * 100))}
          icon={Wallet}
          color="red"
        />
      </div>
